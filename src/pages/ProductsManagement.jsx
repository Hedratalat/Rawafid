...
export default function ProductsManagement() {
  ...
  // جلب المنتجات لايف من Firestore، مرتبة حسب order
  useEffect(() => {
  ...
  // لو عدد الصفحات قل (بعد حذف مثلاً) وأنت واقف في صفحة مش موجودة، ارجع لآخر صفحة متاحة
  useEffect(() => {
    ...
    try {
      ...
      const finalOrder =
        form.order === "" || form.order === null
          ? products.length === 0
            ? 1
            : Math.max(...products.map((p) => Number(p.order) || 0)) + 1
          : Number(form.order);

      // تصفية صور التفاصيل الفاضية قبل الحفظ
  ...
  // تبديل ترتيب منتجين مع بعض (فوق/تحت) - بيشتغل على المصفوفة الكاملة مش الصفحة بس
  async function moveProduct(fullIndex, direction) {
    const targetIndex = direction === "up" ? fullIndex - 1 : fullIndex + 1;
    if (targetIndex < 0 || targetIndex >= products.length) return;

    const current = products[fullIndex];
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      // لو كنت بتعدل نفس المنتج اللي اتحذف، اقفل الفورم
  ...
  return (
    <div className="max-w-3xl mx-auto" dir="rtl">
      ...
      >
        ...
        {/* ===== تصنيفات إضافية: وصل حديثًا / أفضل المنتجات ===== */}
        <div className="flex items-center gap-6">
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            />
            <span className="text-sm font-medium text-darkText flex items-center gap-1">
      ...
      <div className="flex items-center justify-between mb-4">
        <h2 className="font-heading text-xl font-bold text-darkText">
          المنتجات الحالية (بترتيب الظهور)
        </h2>
        {products.length > 0 && (
      ...
      {loading ? (
        ...
        <p className="text-sm text-darkText/60">لا توجد منتجات مضافة بعد.</p>
      ) : (
        <div className="flex flex-col gap-3">
          {paginatedProducts.map((product, i) => {
            const index = startIndex + i; // الإندكس الحقيقي في المصفوفة الكاملة
            const detailImagesCount = Array.isArray(product.images)
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1 flex-wrap">
                    ...
                    {product.isBestSeller && (
                      <span className="flex items-center gap-1 text-[11px] font-bold text-accent bg-accent/10 px-2 py-0.5 rounded-full">
                        <Star size={11} />
                        الأفضل
                      </span>
                    )}
                    {detailImagesCount > 0 && (
                      <span className="flex items-center gap-1 text-[11px] text-darkText/50">
                        <Images size={12} />
