...
function BestSellerCard({ product, onOpen, onAddToCart }) {
  ...
  return (
    <div
      ...
      className={`group cursor-pointer bg-primary/5 border border-primary/10 rounded-3xl overflow-hidden flex flex-col transition-shadow hover:shadow-lg ${
        visible ? "bestseller-fade-up" : "opacity-0 translate-y-4"
      }`}
    >
      <div className="relative aspect-[4/3] bg-gradient-to-br from-secondary/15 via-accent/10 to-primary/10">
        <img
          src={product.image}
          alt={product.name}
          className="w-full h-full object-contain p-6 transition-transform duration-300 group-hover:scale-[1.05]"
        />
      </div>

      <div className="relative -mt-5 bg-white rounded-t-3xl p-4 flex flex-col gap-3 flex-1">
        <div className="min-w-0">
          ...
          {product.description && (
            <p className="text-sm text-darkText/60 leading-6 line-clamp-2 mt-1">
              {product.description}
            </p>
          )}
        </div>
...
export default function BestSellers() {
  ...
  // جلب المنتجات من Products وفلترة اللي isBestSeller بس
  useEffect(() => {
    const q = query(collection(db, "Products"), orderBy("order", "asc"));
    const unsub = onSnapshot(q, (snapshot) => {
      const all = snapshot.docs.map((d) => ({ id: d.id, ...d.data() }));
      setProducts(all.filter((p) => p.isBestSeller));
      setLoading(false);
    });
    return () => unsub();
  }, []);
  ...
  return (
    <section dir="rtl" className="bg-customBg pt-0 pb-14 md:pb-16">
      <style>{`
        @keyframes bestsellerPopIn {
          from { opacity: 0; transform: translateY(10px) scale(0.98); }
          to { opacity: 1; transform: translateY(0) scale(1); }
        }
        @keyframes bestsellerToastIn {
          from { opacity: 0; transform: translateY(-14px); }
          to { opacity: 1; transform: translateY(0); }
        }
        @keyframes bestsellerFadeUp {
          from { opacity: 0; transform: translateY(16px); }
          to { opacity: 1; transform: translateY(0); }
        }
        @keyframes bestsellerHeadingReveal {
          from { opacity: 0; transform: translateY(22px); }
          to { opacity: 1; transform: translateY(0); }
        }
        .bestseller-pop-in { animation: bestsellerPopIn 0.28s ease-out both; }
        .bestseller-toast-in { animation: bestsellerToastIn 0.3s ease-out both; }
        .bestseller-fade-up { animation: bestsellerFadeUp 0.5s ease-out both; }
        .bestseller-heading-reveal { animation: bestsellerHeadingReveal 1.1s ease both; }
      `}</style>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        >
          ...
          <p className="mt-2 text-base text-darkText/60 max-w-md mx-auto">
            اختيارات تكررت طلباتها من عملائنا
          </p>
        </div>
      ...
      <div className="fixed top-5 inset-x-0 z-[300] flex justify-center pointer-events-none px-4">
        {toast && (
          <div className="bestseller-toast-in pointer-events-auto bg-accent text-darkText px-5 py-3 rounded-full shadow-lg text-sm font-bold max-w-[90vw]">
            {toast}
          </div>
        )}
      </div>
      ...
      {selectedProduct && (
        ...
        >
          <div
            ...
            className="bestseller-pop-in bg-white rounded-3xl w-full max-w-3xl max-h-[90vh] overflow-y-auto relative grid grid-cols-1 md:grid-cols-2"
          >
            <button
              onClick={closeProduct}
              aria-label="إغلاق"
